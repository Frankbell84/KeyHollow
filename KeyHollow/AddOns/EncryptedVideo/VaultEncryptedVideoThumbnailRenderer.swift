...
public enum VaultEncryptedVideoThumbnailRenderer {
    ...
    /// asking AVFoundation for a full-resolution frame.
    public static let maximumPixelDimension = 512
    public static let maximumEncodedByteCount = 2 * 1_024 * 1_024

    /// Renders directly from the application-owned protected local file. The
    /// renderer creates no persistent plaintext copy and propagates media,
    /// encoding, and cancellation failures to the lifecycle-owning caller.
...
/// AVFoundation's cancellation entry point is designed for cancellation
/// handlers. This narrowly scoped box makes that cross-executor operation
/// explicit without sharing generated image data or mutable application state.
